...
class PerceptionVM : DJIViewModel() {
    ...
    fun addPerceptionInfoListener() {
        ...
        KeyManager.getInstance().listen(KeyTools.createKey(RadarKey.KeyConnection), this) { _, newValue ->
            //雷达拔掉时会返回null值
            if (newValue == null) {
    ...
    fun setRadarObstacleAvoidanceEnabled(isEnabled: Boolean, direction: PerceptionDirection, callback: CommonCallbacks.CompletionCallback) {
